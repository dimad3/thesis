      ...
      <tbody>
        <?php foreach ($records as $record): ?>
          <tr>
            ...
            <td><?= htmlspecialchars($record->groupname, ENT_QUOTES, 'UTF-8'); ?></td>
            <td>
              ...
              <?php if ($user->hasPermissions('admin')): ?>
                ...
                <?php if ($record->groupid == 1): ?>
                  <a href="permissions.php?id=<?= $record->id ?>" class="btn btn-success">Назначить модератором</a>
                <?php endif; ?>
                <?php if ($record->groupid == 3): ?>
                  <a href="permissions.php?id=<?= $record->id ?>" class="btn btn-danger">Разжаловать модератора</a>
                <?php endif; ?>
              <?php endif; ?>
